...
Description:
    ...
    The procedure performs data cleansing, deduplication, standardization,
    date conversion, missing-value handling, and business-rule validation
    across CRM and ERP datasets.

    It also reports loading progress, inserted row counts, execution time,
...
Notes:
    • Silver tables are truncated before each full load.
    • The dwh_create_date column is populated automatically using its
      default value.
    • If any table load fails, the procedure reports the affected table
